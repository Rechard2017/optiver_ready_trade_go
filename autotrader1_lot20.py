    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            ...
            if abs(self.position) > 50:
                price_adjustment = -1 * np.sign(self.position) * (((abs(self.position)-50) // LOT_SIZE) * TICK_SIZE_IN_CENTS * 2 + 500)
            ...
            if self.p_bid_id == 0 and new_bid_price != 0 and self.position < POSITION_LIMIT:
            ...
            if self.p_ask_id == 0 and new_ask_price != 0 and self.position > -POSITION_LIMIT:
